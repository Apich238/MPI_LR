...
void GenerateMatrix(MatrixPart * m, int msize, IndexInterval myinterval) {
	...
	for (int i = 0, k = myinterval.begin; i < m->RowCount; i++, k++) {
		...
		for (int j = 0; j < m->ColumnCount; j++) {
			...
			if (m->Rows[i].index == j)
				m->Rows[i].values[j] = 100 * m->Rows[i].values[j] +100;
...
int main(int argc, char* argv[]) {
	...
	tCreate += MPI_Wtime() - t;
	double* sol=NULL;
	if (rank == 0) sol = (double*)malloc(sizeof(double)*(MyPart.ColumnCount - 1));
	t = MPI_Wtime();
	Gauss(&MyPart, sol);
	tEvaluate += MPI_Wtime() - t;
	if (rank == 0) WriteResults(sol, MyPart.ColumnCount - 1);
	freeMatrix(&MyPart);
	if (rank == 0) free(sol);
